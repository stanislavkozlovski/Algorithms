...
def main():
    ...
    with open('input.txt', 'r') as f:
        ...
        for line in f:
            weight = int(line)
            words.append(HuffmanTree(root=Node(value=idx, frequency=weight)))
            idx += 1
    ...
    print(min([len(v) for v in test.cache.values()]))
    # get the maximum depth
    print(max([len(v) for v in test.cache.values()]))
    print(test.max_depth - 1)
...
class HuffmanTree:
    ...
    def merge(self, other_tree):
        ...
        if isinstance(other_tree, HuffmanTree):
            self.max_depth = max(other_tree.max_depth, self.max_depth) + 1
